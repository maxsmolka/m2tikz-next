...
%MAKEPGFPLOTSPLAN Plan deterministic TeX and optional scalar-image PNG assets.
    ...
    if strcmp(imageBackend, 'hybrid') && ...
            (~ischar(assetDirectoryName) || ~isrow(assetDirectoryName) || ...
             isempty(assetDirectoryName) || any(strcmp(assetDirectoryName, {'.','..'})) || ...
             any(double(assetDirectoryName) < 32) || any(double(assetDirectoryName) == 127) || ...
             any(ismember(assetDirectoryName, [char(47) char(92) '"%#{}$&^~'])))
        error('M2T2:E055:UnsafeAssetReference', ...
              'Hybrid assets require one safe relative directory name.');
    end
