...
%RUNM54SURFACETESTS Validate the public synthetic M5.4 3-D surface slice.
    ...
    function d=z11(),[f,a,c]=surfaceFigure(7);hold(a,'on');plot3(a,[-1 1],[0 0],[1 1],'k-');set(a,'SortMethod','childorder');kinds=cellfun(@(s)s.kind,read(f).axes{1}.series,'UniformOutput',false);assert(any(strcmp(kinds,'m2t2.surface'))&&any(strcmp(kinds,'m2t2.line3')));compile(f,'Z11');d='surface plus explicitly ordered Plot3 compiled';clear c;end
    ...
    function d=z22(),f=figure('Visible','off');c=onCleanup(@()close(f));a=axes('Parent',f);scatter3(a,1:3,1:3,1:3);view(a,30,20);ir=read(f);assert(strcmp(ir.axes{1}.series{1}.kind,'m2t2.scatter3'));d='bounded scatter3 supported since M6.5';clear c;end
