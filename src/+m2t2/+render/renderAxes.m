...
%RENDERAXES Render normalized axes semantics and discriminated series.
    ...
    for s = 1:numel(node.series)
        ...
        if any(strcmp(node.series{s}.kind,{'m2t2.scatter','m2t2.scatter3'}))
    ...
    if node.dimensionality == 3
        options{end+1}='axis line style={-}';
        options{end+1}='mark layer=like plot';
    ...
    for s = 1:numel(node.series)
        ...
        if strcmp(node.series{s}.kind, 'm2t2.image') && ...
            ...
            item=node.series{s};
            if strcmp(item.kind,'m2t2.scatter3'),item=m2t2.render.orderScatter3(item,node);end
            seriesLines = m2t2.render.renderSeries( ...
                item, colorName(axesIndex, s), colormapName(axesIndex), ...
                node.colorMapping);
        end
