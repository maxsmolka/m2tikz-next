...
%SERIESOPTIONS Produce deterministic base style options for a series node.
    switch node.kind
        ...
        case 'm2t2.scatter'
            ...
            visibility='';
            if strcmp(node.edgeMode,'none'),visibility=[visibility ',draw opacity=0'];end
            if strcmp(node.faceMode,'none'),visibility=[visibility ',fill opacity=0'];end
            options = {'only marks', ...
                       ['mark=' m2t2.render.scatterMarkerName(node.marker, filled)], ...
                       ['mark size=' m2t2.util.formatNumber(node.markerSize(1) / 2) 'pt'], ...
                       ['mark options={solid,draw=' edge ',fill=' face visibility '}'], ...
                       'forget plot'};
        case 'm2t2.errorbar'
            options = lineOptions(node, colorName);
        case 'm2t2.bar'
